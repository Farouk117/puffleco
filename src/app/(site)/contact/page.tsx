...
const info = [
  ...
  { label: "Email", value: "orders@thepufflette.co", href: "mailto:orders@thepufflette.co" },
  { label: "Instagram", value: "@thepufflette.co", href: "https://www.instagram.com/thepufflette.co" },
];
    ...
    <section className="container-page py-14 sm:py-20">
      ...
      <div className="mx-auto mt-12 grid max-w-4xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="flex flex-col gap-4">
          {info.map((item) => (
            <div key={item.label} className="rounded-[1.75rem] border-2 border-brand-line/40 bg-white/80 p-5">
              <p className="text-xs font-black uppercase tracking-[0.2em] text-stone-500">{item.label}</p>
              {item.href ? (
                <a href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="mt-2 block text-lg font-black text-brand-coral-dark">
                  {item.value}
                </a>
              ) : (
